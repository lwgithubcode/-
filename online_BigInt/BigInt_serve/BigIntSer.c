...
void Data_Handler(bigint &bt, bigint &bt1, bigint &bt2, OPER &oper)
{
	...
	else
	{
		while(*data!='\0' && *data!='&')
		{
			if(*data>='0' && *data<='9') bt1.push_front(*data-'0');
				++data;
		}
		data++;
		while(*data!='\0' && *data!='&')
...
void Show_Result(bigint &bt, bigint &bt1, bigint &bt2, OPER oper)
{
	printf("<br>");
	printf("<div style='word-wrap:break-word;font-size:30px;font- weight:blod;' align=center>bt1=");
	for(int i=bt1.size(); i>=1; --i) printf("%d", bt1[i]);
		printf("<br>");
	if(oper == ADD)
		printf("+<br>");
	else if(oper == SUB)
